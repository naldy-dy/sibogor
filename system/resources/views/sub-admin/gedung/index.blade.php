				...
				<table id="example" class="display table-hover table-datatable table" style="width:150%">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<div class="btn-group">
									<button type="button" class="btn btn-primary light" data-toggle="modal" data-target="#detail{{$gedung->id}}"><i class="fa fa-info"></i></button>					
									@include('sub-admin.utils.hapus', ['url' =>url('sub-admin/kategori', $gedung->id)])
								</div>
							</td>
							<td>{{ucwords($gedung->nama)}}</td>
							<td>{{ucwords($gedung->nama_pemilik)}}</td>
							<td>Rp. {{number_format($gedung->harga)}}</td>
							<td>{{$gedung->alamat}}</td>

							<!-- Modal -->
							<div class="modal fade" id="detail{{$gedung->id}}" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdropLabel" aria-hidden="true">
